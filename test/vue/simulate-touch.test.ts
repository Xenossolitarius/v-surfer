import { describe, it, expect } from 'vitest';
import { mount, flushPromises, type VueWrapper } from '@vue/test-utils';
import { h } from 'vue';
import Surfer from '../../src/vue/surfer';
import Item from '../../src/vue/item';

function mountSurfer(props: Record<string, unknown> = {}) {
  return mount(Surfer, {
    attachTo: document.body,
    props: { slidesPerView: 1, spaceBetween: 0, threshold: 0, ...props },
    slots: { default: () => Array.from({ length: 3 }, (_, i) => h(Item, { data: i, key: i })) },
  });
}

const transform = (w: VueWrapper): string =>
  (w.find('.v-surfer-wrapper').element as HTMLElement).style.transform;
const REST = 'translate3d(0px, 0px, 0px)';

function drag(w: VueWrapper, pointerType: string): void {
  const slide = w.findAll('.v-surfer-slide')[0].element;
  slide.dispatchEvent(
    new PointerEvent('pointerdown', {
      pointerId: 1,
      bubbles: true,
      clientX: 200,
      clientY: 100,
      pointerType,
    }),
  );
  slide.dispatchEvent(
    new PointerEvent('pointermove', {
      pointerId: 1,
      bubbles: true,
      clientX: 100,
      clientY: 100,
      pointerType,
    }),
  );
}

describe('simulateTouch', () => {
  it('ignores a mouse drag when simulateTouch is false', async () => {
    const wrapper = mountSurfer({ simulateTouch: false });
    await flushPromises();
    drag(wrapper, 'mouse');
    await flushPromises();
    expect(transform(wrapper)).toBe(REST);
    wrapper.unmount();
  });

  it('allows a mouse drag by default', async () => {
    const wrapper = mountSurfer({});
    await flushPromises();
    drag(wrapper, 'mouse');
    await flushPromises();
    expect(transform(wrapper)).not.toBe(REST);
    wrapper.unmount();
  });

  it('still allows a touch drag when simulateTouch is false', async () => {
    const wrapper = mountSurfer({ simulateTouch: false });
    await flushPromises();
    drag(wrapper, 'touch');
    await flushPromises();
    expect(transform(wrapper)).not.toBe(REST);
    wrapper.unmount();
  });
});
